  ...
  <div>
    <section class="page-header section--dark">
      <div class="container">
        ...
        <p class="page-header__sub">Все фотоотчёты ЖК Атмосфера — без купюр</p>
      </div>
    </section>
      ...
      <div class="container">
        ...
        <div v-else class="empty-state">
          ...
          <NuxtLink to="/jk/atmosfera">
            <AppButton variant="outline" style="margin-top: var(--space-4)">Вернуться к ЖК</AppButton>
          </NuxtLink>
        </div>
      </div>
    </section>
...
useSeoMeta({
  title: 'Ход строительства ЖК Атмосфера | Atamura Group',
  description: 'Фотоотчёты о ходе строительства ЖК Атмосфера в Алматы. Ежемесячные обновления.',
})

useHead({
  htmlAttrs: { lang: 'ru' },
  link: [{ rel: 'canonical', href: `${siteUrl}/jk/atmosfera/progress` }],
})
...
const photos = await useOptionalApi<ProgressPhoto[]>('/api/progress?jk=atmosfera&limit=100')
