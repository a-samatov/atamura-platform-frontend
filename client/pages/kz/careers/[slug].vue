      ...
      <div class="container vacancy-layout">
        ...
        <aside class="vacancy-sidebar">
          <div class="apply-card">
            <h3 class="apply-card__title">Бос орынға өтінім беру</h3>
            <p class="apply-card__desc">Түйіндемені поштаға жіберіңіз — 3 жұмыс күні ішінде жауап береміз</p>
            <a :href="`mailto:hr@atamuragroup.kz?subject=Бос орын: ${job.title}`">
              <AppButton size="lg" style="width: 100%">Түйіндеме жіберу</AppButton>
            </a>
            <p class="apply-card__email">hr@atamuragroup.kz</p>
          </div>
          <div class="other-vacancies">
            <h4 class="other-vacancies__title">Басқа бос орындар</h4>
            <NuxtLink to="/kz/careers" class="other-vacancies__link">Барлық бос орындар →</NuxtLink>
          </div>
        </aside>
      </div>
    </section>
...
if (error.value || !job.value) {
  throw createError({ statusCode: 404, message: 'Бос орын табылмады' })
}

const breadcrumbs = [
  { label: 'Басты бет', href: '/kz' },
  { label: 'Бос орындар', href: '/kz/careers' },
...
const formatSalary = (j: Job) => {
  ...
  if (j.salary_from) return `бастап ${j.salary_from.toLocaleString()} ${currency}`
  if (j.salary_to) return `дейін ${j.salary_to.toLocaleString()} ${currency}`
  return ''
}

useSeoMeta({
  title: `${job.value.title} | Atamura Group`,
  description: `${job.value.title} бос орыны, ${job.value.city}. ${job.value.employment_type}.`,
})

useHead({
  htmlAttrs: { lang: 'kk' },
  link: [
    { rel: 'canonical', href: `${siteUrl}/kz/careers/${slug}` },
