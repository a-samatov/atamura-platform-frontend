<template>
  <div>
    <section class="page-header section--dark">
      <div class="container">
        <AppBreadcrumb :items="[{ label: 'Басты бет', href: '/kz' }, { label: 'Ипотека' }]" />
        <h1 class="page-header__title">Ипотека</h1>
        <p class="page-header__sub">Серіктес банктеріміз арқылы пәтерді тиімді сатып алыңыз</p>
      </div>
    </section>

    <section class="section">
      <div class="container">
        <p class="section-label">Шарттар</p>
        <h2 class="section-title" style="margin-bottom: var(--space-10)">Серіктес банктер</h2>
        ...
        <div class="banks-grid">
          <div class="bank-card" v-for="bank in banks" :key="bank.name">
            ...
            <div class="bank-card__info">
              <div class="bank-stat">
                <span class="bank-stat__value">{{ bank.rate }}%-дан</span>
                <span class="bank-stat__label">жылдық</span>
              </div>
              <div class="bank-stat">
                <span class="bank-stat__value">{{ bank.down }}%-дан</span>
                <span class="bank-stat__label">бастапқы жарна</span>
              </div>
              <div class="bank-stat">
                <span class="bank-stat__value">{{ bank.term }} жылға дейін</span>
                <span class="bank-stat__label">мерзім</span>
              </div>
            </div>
            <AppButton size="sm" @click="uiStore.openModal('application', { bank: bank.name })">Өтінім қалдыру</AppButton>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="section section--brand">
      <div class="container mortgage-cta">
        <div>
          <h2 class="section-title">Ипотека бойынша кеңес керек пе?</h2>
          <p style="margin-top: var(--space-3); color: var(--color-gray-600);">Менеджеріміз сіздің жағдайыңызға сай ең қолайлы шарттарды таңдайды</p>
        </div>
...
useSeoMeta({
  ...
  description: 'ЖК Атмосферадағы пәтерлерге ипотека. Серіктес банктер: Halyk, Kaspi, Jusan, Freedom. Жылдық 15,5%-дан.',
})

useHead({
  htmlAttrs: { lang: 'kk' },
  link: [
    { rel: 'canonical', href: `${siteUrl}/kz/mortgage` },
