  ...
  <div v-if="article" class="article-page">
    <div class="container article-page__inner">
      <aside class="article-sidebar">
        <AppBreadcrumb :items="[{ label: 'Басты бет', href: '/kz' }, { label: 'Жаңалықтар', href: '/kz/news' }, { label: article.title }]" />
        ...
        <div class="article-sidebar__cta">
          <p class="article-sidebar__cta-text">Жылжымайтын мүлікке қызығушылық бар ма?</p>
      ...
      >
        ...
        <footer class="article-body__footer">
          <NuxtLink to="/kz/news" class="article-back">
...
const { data: article, error } = await useAsyncData(
  `news-kz-${route.params.slug}`,
  () => $fetch<NewsArticle>(`${apiBase}/news/${route.params.slug}?lang=kk`),
  {
    server: true,
...
if (error.value || !article.value) {
  throw createError({ statusCode: 404, statusMessage: 'Мақала табылмады' })
}

useSeoMeta({
  title: article.value?.seo_title || article.value?.title || 'Жаңалық',
...
useHead({
  htmlAttrs: { lang: 'kk' },
  link: [
    { rel: 'canonical', href: `${config.public.siteUrl}/kz/news/${route.params.slug}` },
    { rel: 'alternate', hreflang: 'ru', href: `${config.public.siteUrl}/news/${route.params.slug}` },
    { rel: 'alternate', hreflang: 'kk', href: `${config.public.siteUrl}/kz/news/${route.params.slug}` },
  ],
  script: [{
