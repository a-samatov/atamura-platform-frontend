  ...
  <div>
    <section class="page-header section--dark">
      <div class="container">
        ...
        <p class="page-header__sub">Все фотоотчёты Aqsai Resort — без купюр</p>
      </div>
    </section>
...
useSeoMeta({
  title: 'Ход строительства Aqsai Resort | Atamura Group',
  description: 'Фотоотчёты о ходе строительства Aqsai Resort. Ежемесячные обновления.',
})

useHead({
  htmlAttrs: { lang: 'ru' },
  link: [{ rel: 'canonical', href: `${siteUrl}/jk/aqsai/progress` }],
})
...
const photos = await useOptionalApi<ProgressPhoto[]>('/api/progress?jk=aqsai&limit=100')
