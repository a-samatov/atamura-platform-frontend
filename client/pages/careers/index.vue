  ...
  <div>
    <section class="page-header section--dark">
      <div class="container page-header__inner">
        <AppBreadcrumb :items="[{ label: 'Главная', href: '/' }, { label: 'Карьера' }]" />
        <h1 class="page-header__title">Карьера в Atamura Group</h1>
        <p class="page-header__sub">Присоединитесь к команде, которая строит качественное жильё</p>
      </div>
    </section>
    ...
    <section class="section section--light">
      <div class="container">
        ...
        <h2 class="section-title">Работа в Atamura Group</h2>
    ...
    <section class="section">
      <div class="container">
        <p class="section-label">Открытые позиции</p>
        ...
        <div v-else class="vacancies-empty">
          <p>Сейчас открытых вакансий нет. Следите за обновлениями!</p>
        </div>
      </div>
    </section>
    ...
    <section class="section section--brand">
      <div class="container cta-row">
        <div>
          <h2 class="section-title">Не нашли подходящую вакансию?</h2>
          <p style="margin-top: var(--space-3); color: var(--color-gray-600);">Отправьте резюме — мы рассмотрим его при появлении подходящих позиций</p>
        </div>
        <a href="mailto:hr@atamuragroup.kz">
          <AppButton size="lg">Отправить резюме</AppButton>
        </a>
      </div>
    </section>
  </div>
...
useSeoMeta({
  title: 'Карьера | Atamura Group',
  description: 'Вакансии в Atamura Group — строительной компании в Алматы. Присоединитесь к команде профессионалов.',
})

useHead({
  htmlAttrs: { lang: 'ru' },
  link: [
    { rel: 'canonical', href: `${siteUrl}/careers` },
...
const { data: jobs } = await useAsyncData<Job[]>('jobs-ru',
...
const formatSalary = (job: Job) => {
  ...
  if (job.salary_from) return `от ${job.salary_from.toLocaleString()} ${currency}`
  if (job.salary_to) return `до ${job.salary_to.toLocaleString()} ${currency}`
  return ''
}

const perks = [
  { title: 'Конкурентная зарплата', desc: 'Прозрачная система оплаты, квартальные бонусы за результат.', icon: `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>` },
  { title: 'Официальное трудоустройство', desc: 'Полное соблюдение ТК РК, ДМС, оплачиваемый отпуск.', icon: `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>` },
  { title: 'Развитие', desc: 'Корпоративное обучение, оплата профессиональных курсов.', icon: `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>` },
  { title: 'Дружная команда', desc: 'Молодой коллектив, корпоративные мероприятия, открытая культура.', icon: `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>` },
]
</script>
