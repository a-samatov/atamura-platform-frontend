  ...
  <div v-if="article" class="article-page">
    <div class="container article-page__inner">
      <aside class="article-sidebar">
        <AppBreadcrumb :items="[{ label: 'Главная', href: '/' }, { label: 'Новости', href: '/news' }, { label: article.title }]" />
        ...
        <div class="article-sidebar__cta">
          <p class="article-sidebar__cta-text">Интересует недвижимость?</p>
      ...
      >
        ...
        <footer class="article-body__footer">
          <NuxtLink to="/news" class="article-back">
...
const { data: article, error } = await useAsyncData(
  ...
  () => $fetch<NewsArticle>(`${apiBase}/news/${route.params.slug}?lang=ru`),
  {
    server: true,
...
useSeoMeta({
  title: article.value?.seo_title || article.value?.title || 'Новость',
...
useHead({
  htmlAttrs: { lang: 'ru' },
  link: [
    { rel: 'canonical', href: `${config.public.siteUrl}/news/${route.params.slug}` },
  ],
})
