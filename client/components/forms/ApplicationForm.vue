<template>
  <div class="app-form">
    <div class="app-form__header">
      <h2 class="app-form__title">Оставить заявку</h2>
      <p class="app-form__subtitle">Менеджер свяжется с вами в течение часа</p>
    </div>
    ...
    <div v-if="form.success.value" class="app-form__success" role="alert">
      ...
      <p class="app-form__success-title">Заявка отправлена!</p>
...
/>
        ...
        <div v-if="props.jkSlug !== undefined" class="field">
          <label for="app-jk">Жилой комплекс</label>
          <select id="app-jk" v-model="selectedJk">
      ...
      <p class="app-form__legal">
        Нажимая кнопку, вы соглашаетесь с <NuxtLink to="/privacy" class="privacy-link" target="_blank">политикой обработки персональных данных</NuxtLink>
      </p>
    </form>
  </div>
</template>
...
const PHONE_ERRORS: Record<string, string> = {
  ...
  phone_too_short: `Минимум 7 цифр`,
  phone_too_long: 'Неверный номер',
}
...
function validate(): boolean {
  ...
  if (!name.value.trim() || name.value.trim().length < 2) {
    nameError.value = 'Введите имя (минимум 2 символа)'
    valid = false
  }
