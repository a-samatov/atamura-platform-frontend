<template>
  <div class="quiz">
    <div v-if="quiz.success.value" class="quiz__success" role="alert">
      ...
      <p class="quiz__success-title">Заявка отправлена!</p>
      <p>Менеджер подберёт для вас лучший вариант и позвонит в течение часа.</p>
    </div>

    <template v-else-if="!quiz.showContactForm.value">
      <div class="quiz__header">
        <p class="section-label">Подбор квартиры</p>
    ...
    <template v-else>
      <div class="quiz__contact">
        <p class="section-label">Последний шаг</p>
        <h2 class="quiz__contact-title">Оставьте контакты</h2>
        <p class="quiz__contact-sub">Менеджер подберёт лучший вариант и позвонит вам</p>

        <form @submit.prevent="handleContactSubmit" novalidate>
          <div class="app-form__fields">
            <div class="field">
              <label for="quiz-name">Ваше имя</label>
              <input id="quiz-name" v-model="contactName" type="text" placeholder="Асан Асанов" required autocomplete="given-name" />
            </div>
            <div class="field">
              <label for="quiz-phone">Номер телефона</label>
              <input
  id="quiz-phone"
  :value="contactPhone"
...
/>
          ...
          <p class="app-form__legal">Нажимая кнопку, вы соглашаетесь с <NuxtLink to="/privacy" class="privacy-link" target="_blank">политикой обработки персональных данных</NuxtLink></p>
        </form>
      </div>
    </template>
  </div>
