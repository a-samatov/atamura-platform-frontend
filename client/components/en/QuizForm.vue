<template>
  <div class="quiz">
    <div v-if="quiz.success.value" class="quiz__success" role="alert">
      ...
      <p class="quiz__success-title">Request Sent!</p>
      <p>Our manager will find the best option for you and call within an hour.</p>
    </div>

    <template v-else-if="!quiz.showContactForm.value">
      <div class="quiz__header">
        <p class="section-label">Apartment Selection</p>
    ...
    <template v-else>
      <div class="quiz__contact">
        ...
        <h2 class="quiz__contact-title">Leave Your Contact Details</h2>
        <p class="quiz__contact-sub">Our manager will find the best option and call you</p>

        <form @submit.prevent="handleContactSubmit" novalidate>
          <div class="app-form__fields">
            <div class="field">
              <label for="quiz-name-en">Your Name</label>
              <input id="quiz-name-en" v-model="contactName" type="text" placeholder="John Smith" required autocomplete="given-name" />
            </div>
            <div class="field">
              <label for="quiz-phone-en">Phone Number</label>
              <input
  id="quiz-phone-en"
  :value="contactPhone"
...
/>
          ...
          <p class="app-form__legal">By clicking the button, you agree to the <NuxtLink to="/en/privacy" class="privacy-link" target="_blank">personal data processing policy</NuxtLink></p>
        </form>
      </div>
    </template>
  </div>
